...
def is_block_start(line, block_type):
    ...
    if block_type == "if":
        return (line.startswith("if(") or line.startswith("만약(")) and line.endswith(":")
    return False


def is_else(line):
    return line.strip() == "else:" or line.strip() == "아니면:"
...
def find_comparison_operator(expr, op):
    quote = None
    i = 0
...
def eval_condition(expr, lang):
    expr = expr.strip()
    ops = ["==", "!=", ">=", "<=", ">", "<"]

    for op in ops:
        idx = find_comparison_operator(expr, op)
    ...
    return bool(eval_value(expr, lang))
...
def execute_lines(lines, lang):
    ...
    while i < len(lines):
        ...
        if is_block_start(trimmed, "if"):
            condition = get_inside(trimmed, "(", ")")
            if_block, if_end = collect_block(lines, i)

            next_index = if_end + 1
            else_block = None

            if next_index < len(lines) and is_else(lines[next_index]):
                else_block, else_end = collect_block(lines, next_index)
                i = else_end + 1
            else:
                i = if_end + 1

            if eval_condition(condition, lang):
                execute_lines(if_block, lang)
            elif else_block is not None:
                execute_lines(else_block, lang)
...
def show_help():
    ...
    wprint("  whale shutdown  : Exit Whale Terminal")
    wprint("")
    wprint("Examples")
    wprint("  print['hello whale']")
    wprint("  age = int(input('Age >>> '))")
    wprint("  if(age >= 14):")
    wprint("      print['OK']")
    wprint("  else:")
    wprint("      print['NO']")
...
def handle_command(value):
    ...
    if value == "exit" or value == "whale shutdown":
        raise SystemExit
...
def boot():
    ...
    wprint("Commands: open, editor, help, clear, whale shutdown")
    wprint("New in 0.2: if/else, 만약/아니면, comparison operators")
    wprint("")
...
def main():
    ...
    while True:
        try:
            ...
            wprint("\nUse whale shutdown to close Whale Terminal.")
